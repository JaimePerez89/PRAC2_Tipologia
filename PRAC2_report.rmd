...
model <- glm(diagnosis ~ texture + radius + perimeter + area + scale(smoothness) + compactness + concavity + scale(concave_points) + symmetry + fractal_dimension, data = data, family = binomial)
#le he puesto una función de scale porque he visto que da número más "normales", pero no tengo muy claro si es está bien del todo
tab_model(model, show.r2 = FALSE) #añado esta función para representar los datos, me parece más limpia que la del summary
#summary(model)
```
...
```{r}
library(ROCR)
pred1 <- prediction(as.numeric(pred_valid_DT), as.numeric(test_set$diagnosis))
