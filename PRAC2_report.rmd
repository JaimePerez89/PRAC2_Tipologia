...
En nuestro caso utilizaremos como factor de grupos la clasificación del tumor (maligno vs benigno). Realizaremos un F-test, también conocido como contraste de la razón de varianzas, cuyo objetivo será contrastar la hipótesis nula de que dos poblaciones normales tengan la misma varianza.
...
### ¿Qué variables influyen más en la peligrosidad del tumor?

Para dar respuesta a la pregunta, se utilizará un modelo de regresión logística:
...
# Creación del modelo
model <- glm(diag_num ~., data = data_model, family = binomial)
summary(model)
```
...
### ¿El tamaño de los núcleos influye en la clasificación del tumor?



### ¿La irregularidad de los núcleos influye en la clasificación del tumor?
...
¿Apartado específico o vamos representando gráficamente los resultados en cada uno de los anteriores?
